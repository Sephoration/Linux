...
int main(int argc,char const *argv[]) {
    ...
    */
    ...
    const char *__restrict __fmt   #带格式化的字符串
        fmt: 格式化字符串
        ...: 可变参数，填入格式化的长字符串
    return:  成功返回写入的字符数，失败返回负值
    fprintf (FILE *__restrict __stream, const char *__restrict __fmt, ...)
    */
    char *name = "Hello 尚硅谷 Linux C 语言";
    int printf_1 = fprintf(fp , "\n 我想说： %s", name); // 向文件中写入内容
    if (printf_1 == EOF) {
        perror("fprintf");
        return -1;
    }
    else {
        printf("fprintf 成功写入 %d 个字符！\n",printf_1);
    }
